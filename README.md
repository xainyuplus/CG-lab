...
└── src/
    ...
    ├── work2/           # 实验2：MVP变换与3D立方体
        ...
        └── main.py      # 实验2的主程序入口
    ├── work5/           # 实验5：Whitted-Style光线追踪
        ├── __init__.py
        ├── config.py    # 场景、材质和渲染参数
        ├── physics.py   # 求交、阴影、着色与迭代光线弹射
        └── main.py      # 交互式渲染入口
    ├── work6/           # 实验6：可微渲染
        ├── __init__.py
        ├── config.py    # 目标场景、光源和优化参数
        ├── physics.py   # 可微 Ray Casting、Loss 和 Adam 更新
        └── main.py      # 左右对比可视化入口
    └── work7/           # 实验7：质点弹簧布料
        ├── __init__.py
        ├── config.py    # 布料、弹簧、积分和相机参数
        ├── physics.py   # 结构弹簧力、速度钳制和三种积分器
        └── main.py      # GGUI 3D 渲染与交互入口
```
...
| 实验 5 | Whitted-Style光线追踪 | [目录](src/work5/) / [详情](#实验-5whitted-style-光线追踪) |
| 实验 6 | 可微渲染 | [目录](src/work6/) / [详情](#实验-6可微渲染) |
| 实验 7 | 质点弹簧布料 | [目录](src/work7/) / [详情](#实验-7质点弹簧布料) |
...
## 实验 5：Whitted-Style 光线追踪

### 概述

本实验在 Taichi GPU Kernel 中实现迭代式 Whitted-Style 光线追踪。场景由黑白棋盘地面、红色漫反射球和银色镜面球组成，支持硬阴影、Phong 局部光照以及最多五次的镜面光线弹射。

### 核心功能

- **隐式几何求交**：通过解析方程计算光线与无限平面、两个球体的最近交点
- **材质系统**：使用对象 ID 区分棋盘地面、漫反射球和理想镜面球
- **迭代光线追踪**：使用固定上限循环代替递归，维护光线吞吐量并逐次衰减
- **硬阴影**：从漫反射交点向点光源发射阴影射线，判断光源是否被遮挡
- **防止自相交**：新射线起点沿法线偏移 `1e-4`，消除 Shadow Acne
- **实时交互**：可调节点光源三维位置和最大弹射次数
...
|------|------|------|
| 地面 | `y = -1.0` | 黑白棋盘格漫反射 |
| 左球 | 中心 `(-1.5, 0, 0)`，半径 `1.0` | 红色漫反射 |
| 右球 | 中心 `(1.5, 0, 0)`，半径 `1.0` | 银色理想镜面 |

### 技术实现

每个像素首先从相机发射主射线，然后在 GPU Kernel 中进行最多五轮迭代：

1. 计算光线与场景的最近交点。
2. 若未命中物体，将背景色乘以当前吞吐量写入最终颜色。
3. 若命中镜面球，根据 `R = D - 2(D·N)N` 更新反射方向，并将吞吐量乘以镜面反射率。
4. 若命中漫反射物体，执行阴影测试与 Phong 着色，累积颜色后结束该像素的追踪。

棋盘格通过交点的 `x`、`z` 坐标整数部分奇偶性生成，不依赖外部纹理。阴影区域只保留环境光，未遮挡区域叠加漫反射和镜面高光。
...
- `Light X / Light Y / Light Z`：调整点光源位置，观察高光和阴影变化
- `Max Bounces`：调整最大弹射次数，范围为 `1 ~ 5`，默认为 `3`

### 核心文件

| 文件 | 说明 |
|------|------|
| `src/work5/config.py` | 窗口、相机、场景、材质和光照参数 |
| `src/work5/physics.py` | 几何求交、硬阴影、Phong 着色和迭代光线追踪 Kernel |
| `src/work5/main.py` | Taichi GGUI 窗口、滑动条和实时渲染循环 |

### 效果展示

![demo](./assets/videos/实验五.gif)
...
本实验使用 Taichi 自动微分实现一个最小可微渲染闭环：先用目标光源 `(0.8, 0.8, 0.2)` 渲染球体目标图，再从偏差较大的初始光源 `(0.2, 0.2, 0.8)` 出发，通过 MSE Loss 和 Adam 优化器反向更新光源位置。

### 核心功能

- **Ray Casting 正向渲染**：对每个像素发射相机射线，解析求交中心为 `(0.5, 0.5, 0.5)`、半径为 `0.3` 的球体
- **Leaky Lambertian 光照**：使用 `max(ndotl, alpha * ndotl)` 保留背光面的梯度，避免普通 Lambertian 在暗面直接梯度消失
- **Taichi AutoDiff**：将 `light_pos` 声明为 `needs_grad=True` 的可微向量 Field，并用 `ti.ad.Tape(loss)` 反向传播
- **Adam 参数更新**：使用一阶、二阶动量平滑更新光源坐标，减少普通 SGD 后期震荡
- **过程可视化**：窗口左侧显示目标图像，右侧显示当前优化结果，并实时输出 Loss 和光源坐标
...
| 文件 | 说明 |
|------|------|
| `src/work6/config.py` | 窗口、球体、目标光源、初始光源和 Adam 参数 |
| `src/work6/physics.py` | 可微 Ray Casting、Leaky Lambertian、MSE Loss、Adam 更新和左右拼图 |
| `src/work6/main.py` | Taichi GGUI 窗口、优化循环和控制面板 |

### 效果展示

![demo](./assets/videos/实验六.gif)
...
本实验使用 Taichi GGUI 实现一个结构弹簧布料系统。布料被离散为 `24 x 24` 个质点，水平和竖直相邻质点之间连接结构弹簧，并提供显式欧拉、半隐式欧拉和定点迭代近似的隐式欧拉三种积分模式。

### 核心功能

- **质点-弹簧建模**：初始化布料质点、结构弹簧拓扑、三角面片索引和固定点
- **弹力与阻尼力**：按胡克定律计算弹簧力，并用速度方向阻尼抑制能量发散
- **GPU 并行安全**：弹簧两端受力使用 `ti.atomic_add` 累加，避免多线程写入冲突
- **速度钳制**：通过 `clamp_velocity()` 限制最大速度，降低显式欧拉模式下的数值爆炸风险
- **三种积分器**：分别实现 `step_explicit()`、`step_semi_implicit()` 和 `step_implicit_iter()`
- **GGUI 交互**：支持按钮和键盘切换积分方法、暂停、继续和重置布料
...
| 文件 | 说明 |
|------|------|
| `src/work7/config.py` | 布料分辨率、弹簧参数、时间步、相机和光照配置 |
| `src/work7/physics.py` | 初始化、结构弹簧力、原子累加、速度钳制和三种积分器 |
| `src/work7/main.py` | Taichi GGUI 3D 场景、控制面板和模拟循环 |

### 效果展示

![demo](./assets/videos/实验七.gif)
